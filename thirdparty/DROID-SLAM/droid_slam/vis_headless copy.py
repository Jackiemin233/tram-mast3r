...
def compute_scales(video, smpl, traj, device="cuda:0"):
    ...
    with torch.no_grad():
        ...
        # Convert poses to 4x4 matrices
        traj =  torch.index_select(traj, 0, dirty_index)
        poses = torch.index_select(video.poses, 0, dirty_index)
        disps = torch.index_select(video.disps, 0, dirty_index)
        ...
        # NOTE: Select SMPL - NJ
        '''
            The tstamp record the frame indexes of key frames.
        '''
        ...
        smpl_l['pred_verts_cam'] = smpl_l['pred_verts'] + smpl_l['pred_trans'] # 2D 
        #smpl_l['pred_verts_wc'] = smpl_l['pred_verts_cam'] @ R + T.transpose(0,2,1)
        
        # Save the mesh for visualization
        for idx in range(smpl_l['pred_verts_cam'].shape[0]):
            trimesh.Trimesh(smpl_l['pred_verts_cam'][idx], smpl[0]['smpl_faces']).export(f'/hpc2hdd/home/gzhang292/nanjie/project4/tram/vis_smpl/smpl_{idx}.obj')

        # pred_cam_t = torch.index_select(torch.tensor(traj[:, :3]), 0, tstamp.cpu().long())
        # pred_cam_q = torch.index_select(torch.tensor(traj[:, 3:]), 0, tstamp.cpu().long())
        ...
        # smpl_l['pred_verts_wc'] = smpl_l['pred_verts_cam'] @ R + T.transpose(0,2,1)
        # Debug
        # trimesh.Trimesh(smpl_l['pred_verts_wc'][0], smpl[0]['smpl_faces']).export(f'/hpc2hdd/home/gzhang292/nanjie/project4/tram/vis_smpl/smpl_wc_0.obj')
        # trimesh.Trimesh(pred_vert_w[0], smpl[0]['smpl_faces']).export(f'/hpc2hdd/home/gzhang292/nanjie/project4/tram/vis_smpl/smpl_wc_0.obj')
        #============================================================================================================
        points = droid_backends.iproj(SE3(poses).inv().data, disps, video.intrinsics[0]).cpu()
