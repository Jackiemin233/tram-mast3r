...
def compute_scales(video, smpl, traj, device="cuda:0"):
    ...
    with torch.no_grad():
        ...
        # Convert poses to 4x4 matrices
        traj =  torch.index_select(torch.from_numpy(traj).cuda(), 0, dirty_index)
        poses = torch.index_select(video.poses, 0, dirty_index)
        disps = torch.index_select(video.disps, 0, dirty_index)
        ...
        #points = droid_backends.iproj(SE3(traj).inv().data, disps, video.intrinsics[0]).cpu()

        # Depth filtering
        thresh = 0.3 * torch.ones_like(disps.mean(dim=[1, 2]))
        count = droid_backends.depth_filter(video.poses, video.disps, video.intrinsics[0], dirty_index, thresh)

        #count = droid_backends.depth_filter(traj, video.disps, video.intrinsics[0], dirty_index, thresh)
