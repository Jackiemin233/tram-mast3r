...
def visualize_tram(seq_folder, floor_scale=2, bin_size=-1, max_faces_per_bin=30000):
    ...
    for i in range(max_track):
        ...
        pred = smpl(body_pose=pred_rotmat[:,1:], 
                    ...
                    betas=pred_shape, 
                    transl=pred_trans.squeeze(), # Add transl directly
                    pose2rot=False, 
                    default_smpl=True) 
        pred_vert = pred.vertices # trimesh.Trimesh(pred_vert.cpu()[0], smpl.faces).export('/hpc2hdd/home/gzhang292/nanjie/project4/tram/vis/smpl.obj')
        ...
        pred_vert_w, pred_j3d_w = traj_filter(pred_vert_w.cpu(), 
                                            pred_j3d_w.cpu()) # trimesh.Trimesh(pred_vert_w, smpl.faces).export('/hpc2hdd/home/gzhang292/nanjie/project4/tram/vis/smpl_w.obj')
