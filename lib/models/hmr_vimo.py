...
class HMR_VIMO(nn.Module):
    ...
    def inference(self, imgfiles, boxes, img_focal=None, img_center=None, valid=None, frame=None):
        ...
        for frame_ck, boxes_ck in zip(frame_chunks, boxes_chunks):
            ...
            pred_cam.append(results['pred_cam']) # [frame_number, 144]
            ...
            pred_verts.append(results['pred_verts'])
    ...
    def inference_chunk(self, imgfiles, boxes, img_focal, img_center, device='cuda'):
        ...
        for i in tqdm(range(len(db))):
            ...
            pred_trans.append(out['trans_full'].cpu())
            pred_verts.append(out['pred_verts'].cpu())
            scale.append(out['scale'].cpu())
            center.append(out['center'].cpu())
    ...
    def project(self, points, pred_cam, center, scale, img_focal, img_center, return_full=False): # 3D project to 2D
