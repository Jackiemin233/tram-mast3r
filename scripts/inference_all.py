...
from lib.camera import run_metric_slam, calibrate_intrinsics, align_cam_to_world, run_smpl_metric_slam
...
def main(args):
    ...
    # bgmasks = torch.from_numpy(bgmasks)     #cv2.imwrite('/hpc2hdd/home/gzhang292/nanjie/project4/tram/vis/bgmask.png', bgmasks[0].numpy() * 255)

    # Calibrate the intrinsics - NJ Step1
    cam_int, is_static = calibrate_intrinsics(img_folder, masks, is_static=args.static_camera) # camera intrinsics
    #==========================================================
    ...
    for k, trk in enumerate(tracks): #6 persons in demo in total
        ...
        if k+1 >= args.max_humans:
            break
    #==========================================================
    cam_R, cam_T = run_smpl_metric_slam(img_folder, masks=masks, calib=cam_int, is_static=is_static, smpls=results_persons)
